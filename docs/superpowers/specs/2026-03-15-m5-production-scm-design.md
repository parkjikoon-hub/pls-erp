...
### 3.5 수주 시 원자재 부족 자동 판단 (MRP 연동)
```
수주→작업지시서 전환 시점
  → 수주 라인의 각 품목에 대해 BOM 트리 전개
  → 총 소요 원자재 수량 계산 (품목별 합산)
  → 원자재 창고(WH-RAW) 가용 재고와 비교
  → 부족한 원자재 목록 생성:
     - 품목명, 필요 수량, 현재 재고, 안전재고(기본재고), 부족 수량
  → 결과를 응답에 포함 → 프론트엔드에서 경고 배너 표시
```

API 연동:
1. `POST /production/work-orders/from-order/{order_id}` 응답에 `material_shortage` 포함
2. `POST /production/inventory/check-order/{order_id}` — 수주 기준 소요량 사전 조회
3. 대시보드 부족 재고 카드에 "수주 이행 불가 경고" 표시

> **기본 재고 수량 설정**: 사용자가 품목 마스터(M1 시스템 > 품목 관리)에서
> `안전재고` 필드를 직접 설정합니다. 이 값이 "기본 재고 수량"이 되며,
> 원자재 재고가 이 값 아래로 떨어지면 발주 필요 알림이 표시됩니다.
> 수주가 들어오면 "이 주문을 만들려면 원자재가 얼마나 필요하고,
> 지금 얼마 있고, 얼마가 부족한지" 자동 계산합니다.
...
- POST /check-order/{order_id} — 수주 기준 원자재 소요량 사전 조회 (부족분 포함)
