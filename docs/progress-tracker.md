...
- **대표 검토 대응**: Step 1~7 완료 (엑셀 수정, 글자통일, 인사급여, 재무, 생산, 영업, 대시보드/알림)
- **배포**: Render 통합 배포 (프론트+백엔드) — https://pls-erp-api.onrender.com
- **시뮬레이션**: 전체 모듈 API 정상 동작 확인 (2026-03-17)
- **UI 디자인**: ~~시안 C~~ → **리디자인 A2-v2 구현 완료** (2026-03-19)
- **UI 정리**: BackButton 전체 제거 + 배지 크기 통일 + 모듈별 색상 통일 (2026-03-19)
- **PWA**: 앱 아이콘 + 설치 지원 + 서비스워커 (2026-03-19)
...
- [x] M4(재무), M6(그룹웨어), M7(알림) 페이지 글자 크기 통일 완료 (2026-03-19, 13개 파일 85개 변경)

### 근태+결재 연동 (2026-03-17 완료)
- [x] **전자결재에서 연차/반차/조퇴 승인 → 근태 자동 생성 + 연차 차감**
  - 기안 작성 폼에 휴가 유형 3개 추가 (leave/half_leave/early_leave)
  - 휴가 전용 필드: 시작일/종료일, 오전·오후 반차, 일수 자동 계산
  - 백엔드: `_create_attendance_from_approval()` 헬퍼 — 최종 승인 시 AttendanceRecord 생성
  - Employee ↔ User 매칭, 날짜 범위 근태 생성, 연차 잔여일수 차감
  - 결재 상세 페이지에 휴가 정보 카드 표시
  - 파일: ApprovalFormPage.tsx, approval_service.py, ApprovalDetailPage.tsx

### 자동발주 시스템 연동 검토 (2026-03-17 계획 수립)
- [x] 자동발주 시스템(Firebase/Firestore) 분석 보고서 검토
- [x] **ERP Pull 방식** 연동 계획 수립 → `docs/auto-order-integration-plan.md` v2.0
- [ ] 실제 연동 구현 (보류 — PDF OCR 검증 후 진행)

### 전체 프론트엔드 UI 리디자인 (2026-03-18 목업 확정 → 2026-03-19 구현 완료)
- [x] UI 참고 이미지 기반 시안 3종 제작 (A/B/C)
- [x] 시안 A "Clean Compact" 선택 → A1/A2 개선안 2종 제작
- [x] A2 선택 → v2 반복 개선 (사이드바/헤더/탭/필터/배지/테이블/다크모드)
- [x] **최종 목업 확정**: `docs/ui-mockups/redesign-A2-v2.html`
- [x] React 컴포넌트 구현 (Sidebar.tsx, Header.tsx, TabBar.tsx, AppLayout.tsx)
- [x] CSS 변수 교체 + 폰트 변경 (index.css)
- [x] 라우팅 변경 (App.tsx — 모듈 메인 → 세부 페이지 리다이렉트)
- [x] 36개 페이지 상단 영역 통일 (검색/필터/배지/테이블 스타일)
- [x] TypeScript 빌드 + Render 배포

### UI 정리 — BackButton 제거 + 배지/색상 통일 (2026-03-19 완료)
- [x] **돌아가기(BackButton) 버튼 전체 제거** — 27개 페이지 + BackButton.tsx 삭제
- [x] **배지(Badge) 크기 통일** — 13개 페이지 `min-w-[3.5rem] text-center` 적용
- [x] **모듈별 버튼 색상 통일** — 탭 색상과 액션 버튼 색상 일치
  - 시스템관리: emerald → blue (4페이지)
  - 생산관리: emerald/blue → red (5페이지)
  - 영업관리: blue/indigo → emerald (4페이지)
  - 그룹웨어: amber/violet → cyan (5페이지)
- [x] TypeScript 빌드 + Render 배포 (3회)

### 대표 검토 요청 대응 (2026-03-19 Step 1~7 완료)
- [x] **Step 1: 엑셀 다운로드 500 에러 수정** — 한국어 파일명 URL 인코딩 (4개 라우터)
- [x] **Step 2: M4/M6/M7 글자 크기 통일** — text-xs → text-sm (13개 파일, 85개 변경)
- [x] **Step 3: 인사급여(M3) 개선** (✅ 전체 완료)
  - ✅ 4대보험 선택적 적용 (Employee 4 bool + 급여계산 반영)
  - ✅ 추가근무수당 입력 UI (시간→1.5배 자동계산)
  - ✅ 급여 개별/전체 승인 UI — 체크박스+개별승인+전체승인 (`7bd578c`, 2026-03-20)
- [x] **Step 4: 재무회계(M4) 개선** — 매입 세금계산서 "발행"→"등록" 용어 변경
- [x] **Step 5: 생산관리(M5) 개선**
  - 작업지시서 리스트뷰에 수주번호 칼럼 추가
  - 출하 테이블 인라인 상태변경 버튼 추가
- [x] **Step 6: 영업관리(M2) — 견적 RFQ 업로드**
  - 거래처 견적요청서 파일 업로드 + Gemini Vision OCR → 견적서 라인 자동 채우기
- [x] **Step 7: 대시보드 + 알림센터(M6/M7)**
  - 대시보드 하단 공지사항 위젯 (최근 5건)
  - 공지사항 등록 시 알림센터 자동 알림
- [x] **추가: PWA 앱 아이콘** — manifest.json + sw.js + PNG 아이콘 8종
- [x] **추가: DB 컬럼 자동 마이그레이션** — ALTER TABLE 5개 컬럼 (main.py lifespan)
- [x] **추가: Render 배포 문제 해결** — 옛 dist 파일 삭제 + .gitignore dist 주석 처리
- [x] **추가: 전체 테이블 글자체 통일** — 16개 파일 font-mono 제거 (`3781a93`)
- [x] ✅ **급여 개별/전체 승인 UI** — 체크박스+개별승인+전체승인 (`7bd578c`, 2026-03-20)
- [x] ✅ **작업지시서 수주 상세 팝업** — 수주번호 클릭 시 거래처/품목/납기 상세 표시 (`a5fc556`)
- [x] ✅ **세무신고 안내문 상세화** — CSV 구성, 홈택스 절차, 신고기한, 주의사항 (`a5fc556`)
- [x] ✅ **매입 세금계산서 파일 업로드** — PDF/이미지 첨부/다운로드/삭제 (DB+API+UI) (`a5fc556`)
