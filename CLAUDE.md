...
## 이번 세션에서 완료한 작업 (코드 검증 완료)
- ✅ {작업명} — 검증 파일: {실제 확인한 소스 파일 경로}
- ❌ {작업명} — **미구현** (계획만 존재, 코드에 없음)
...
인수인계 문서 작성 시 "완료" 항목에 대해 반드시 아래 검증 절차를 거칩니다:

1. **코드 직접 확인**: 각 "완료" 항목의 핵심 파일(페이지, 서비스, 라우터, API)을 Read 도구로 읽어 실제 구현 여부 확인
2. **계획 ≠ 완료**: 계획서(plan)나 커밋 메시지만으로 완료 판정 금지. 반드시 소스 코드 기준
3. **검증 불가 시**: "⚠️ 미검증" 표기 (컨텍스트 부족 등)
4. **미구현 발견 시**: "❌ 미구현" 표기 + 미완료 작업 목록에 포함
5. **progress-tracker.md 동기화**: 인수인계 문서와 progress-tracker.md의 완료/미완료 상태 일치 확인
