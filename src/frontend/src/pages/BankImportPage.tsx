...
export default function BankImportPage() {
  /* ── 상태 ── */
  const [bankCode, setBankCode] = useState('shinhan');
  ...
  const handleFileUpload = async (file: File) => {
    setParsing(true);
    try {
      const result = await parseBankCSV(file, bankCode);
    ...
    <div className="space-y-4">
      ...
      <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 text-sm text-amber-800">
        ...
        <ol className="list-decimal list-inside space-y-0.5 text-amber-700">
          <li>인터넷뱅킹에서 거래내역 CSV 파일을 다운로드하세요</li>
          <li>아래에서 은행을 선택하고 파일을 업로드하세요</li>
      ...
      <div className="bg-white rounded-xl border border-(--border-main) p-5 space-y-4">
        <div className="flex gap-4 items-end">
          <div className="flex-1 max-w-xs">
            <label className="block text-sm font-medium text-slate-600 mb-1">은행 선택</label>
